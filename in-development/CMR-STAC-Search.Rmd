...
-  This tutorial has been tested on Windows using R Version 4.4.1 and RStudio 
   version 2024.09.0. 
...
For listing the various STAC catalogs associated with NASA CMR, we'll use the following R file, 
which is supplemental to the `earthdatalogin` package. Functions within this R file 
will be utilized in section 2a.
...
Here, we will use the function `list_nasa_stacs`, which connects to the CMR-STAC landing page
(https://cmr.earthdata.nasa.gov/cloudstac/) for cloud datasets. The landing page contains all the available cloud data 
providers and their STAC endpoint. This function uses the `httr` package to navigate the CMR-STAC API.

`jsonlite` is used to change the format of the content returned from our 
...
from the CMR catalog using the `get_nasa_stac_url` function.
...
collections. Below, connect to the LPCLOUD STAC Catalog endpoint using the
`get_nasa_stac_url` function and print the information contained in the Catalog.
...
Let's begin using the `rstac` package. `rstac` is a library that allows users to
interact with STAC Catalogs and their associated data.
Using our LPCLOUD link from section 2a, we'll create an `rstac_query` object.
```{r}
lpcloud_link <- stac(lpcloud_cat_link)
lpcloud_link
```
Next, get the content describing the collections within the LPCLOUD Catalog. 
Important information such as data collection ID and the title are
provided here.
...
In CMR, the Collection ID is used to query by a specific product, so be sure to 
save the ID for a collection you are interested in. For instance, the Collection 
ID for ASTER Global Digital Elevation Model V003 is `ASTGTM_003`. Note that the 
"id" shortname is in the format: productshortname_VVV (where VVV = product version).

Here, we use the short name `ASTGTM_003` to query the STAC Collection. If you are 
...
col_items
```
We can see that the output includes the number of items associated with the collection,
the assets, and fields.
...
Using our STAC Query, let's get the first items within the `ASTGTM_003` STAC Collection.
...
downloadable and streamable URL to data files along with other asset objects. Below, 
the first Granule ID is used to get the downloadable data file, which can be found in the 'href' field.
...
As a reminder, our provider is "LPCLOUD" and we are using the following
API link:
...
- Query by collection: Collection IDs should be defined as a character.
...
Next, submit a query to the STAC Search endpoint.
