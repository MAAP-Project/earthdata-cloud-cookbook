...
```{r}
lpcloud_cat <- stac(lpcloud_cat_link) |>
  get_request()
...
Using our LPCLOUD link from section 2a, we'll use `rstac` to query the LPCLOUD catalog, and then get the content describing the associated collections. Important information such as data collection ID and the title are provided here.
...
STAC Item represents data and metadata assets that are spatiotemporally coincident. Using our STAC Query, let's get the first item within the `ASTGTM_003` STAC Collection.

```{r}
first_item <- stac_query$features[[1]]
first_item
```
